...
def cal_f1s_naive(y_trues, y_scores, args):
    ...
    class_colunms = [str(i) for i in range(max(args.classes, max(unique_labels)+1))]
    classes_f1 = report_df.loc[['f1-score'], class_colunms].values[0]
    f1_all = report_df.loc[['f1-score'], :].values[0]

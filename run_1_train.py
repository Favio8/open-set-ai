...
import shutil
from loss_func import MCMILoss, OpenMaxSeparation, PseudoLabelGenerator, SupConLoss, CELoss
import time
...
def train(dataloader, net, args, criterion, epoch, scheduler, optimizer, device):
    ...
    for step_k, (view1, view2, labels) in enumerate(tqdm(dataloader)):
        ...
        # -------- 前向传播（双视角）--------
        emb1, cls1, out1 = net(view1)
        emb2, cls2, out2 = net(view2)

        # -------- 计算loss - MCMILoss初始阶段 --------
        # 对两个视图取平均损失
        loss1, loss1_sup, loss1_ce = criterion.forward_initial_stage(emb1, out1, labels)
        loss2, loss2_sup, loss2_ce = criterion.forward_initial_stage(emb2, out2, labels)
        loss = (loss1 + loss2) / 2
        ...
        output_list.append(out1.data.cpu().numpy())
        ...
        pd.DataFrame(trn_label).to_csv(args.trn_y_path, index=False, header=False, mode='a')

        # -------- TSNE（必须在循环内！）--------
        if args.whether_tsne:
            emb_np = emb1.detach().cpu().numpy()
            pd.DataFrame(emb_np).to_csv(args.TSNEemb_trn_path, index=False, header=False, mode='a')

    print('Loss: %.4f' % running_loss)

    # ✅ 防止除0 + 修正计数
    loss_avg = running_loss / (steps_num + 1)
...
def evaluate(dataloader, net, args, criterion, device, optimizer):
    ...
    for step_j, (data, labels) in enumerate(tqdm(dataloader)):
        ...
        emb, cls, output = net(data)

        # -------- 计算loss - MCMILoss初始阶段  --------
        loss, _, _ = criterion.forward_initial_stage(emb, output, labels)
        running_loss += loss.item()
...
if __name__ == "__main__":
    ...
    # ---------
    args.leads = 12
    # CPSC18: 原始6类(1,2,4,5,7,8)，去掉未知类4后5个seen类，增强后10类
    args.leads = 12
    args.classes = 10  # 5个seen类 × 2
    args.seen_classes_num = 4  # 5 - 1 = 4个已知类
    args.Hz = 500
    args.duration = 30
    args.length = args.Hz * args.duration
    unseen_class_name = 4  # 类别4(LBBB)为未知类
    # ---------
    ...
    # ----------------------
    if not os.path.exists(label_csv_stage_1):
   ...
   # ------- for multi-class classification -----------
    from loss_func import MCMILoss
